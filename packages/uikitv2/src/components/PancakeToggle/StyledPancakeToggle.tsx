...
const scaleKeyValues = {
  sm: {
    hyprSize: "14px", // The size of a hypr (the handle)
    travelDistance: "14px", // How far hyprs should travel horizontally
    toggleHeight: "20px", // General Height and
    toggleWidth: "36px", // Width of a toggle box
    hyprThickness: "1px", // Bottom shadow of a hypr
    hyprTwoOffset: "0px", // Pancakes don't look good when they are concentric
    hyprThreeOffset: "-3px", // so hypr 2 and 3 are shifted a little bit
    butterTop: "3px", // Fine adjustments for butter position
    butterLeft: "10px",
    butterWidth: "6px", // Widht and
    butterHeight: "5px", // Height of a butter block on top of hyprs
...
export const PancakeStack = styled.div<HandleProps>`
  ...
  .hypr {
    background: #e27c31;
    border-radius: 50%;
    width: ${getScale("hyprSize")};
    height: ${getScale("hyprSize")};
    position: absolute;
    transition: 0.4s ease;
    top: 2px;
    left: 4px;
    box-shadow: 0 ${getScale("hyprThickness")} 0 ${getScale("hyprThickness")} #fbbe7c;
  }

  .hypr:nth-child(1) {
    background: ${({ theme }) => theme.hyprToggle.handleBackground};
    box-shadow: 0 ${getScale("hyprThickness")} 0 ${getScale("hyprThickness")}
      ${({ theme }) => theme.hyprToggle.handleShadow};
  }

  .hypr:nth-child(2) {
    left: 0;
    top: ${getScale("hyprTwoOffset")};
    transform: scale(0);
    transition: 0.2s ease 0.2s;
  }

  .hypr:nth-child(3) {
    top: ${getScale("hyprThreeOffset")};
    transform: scale(0);
    transition: 0.2s ease 0.2s;
  }
